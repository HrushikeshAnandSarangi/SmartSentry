...
def on_message(client, userdata, msg):
    try:
        ...
        if msg.topic == SENSOR_TOPIC:
            ...
            print(f"--- Received ANOMALY ALERT: {payload.get('failure_type')} ---")
...
def insert_alert_data(data):
    # Update this function to include failure_type
    sql = """INSERT INTO alerts(unit_number, time_in_cycles, phase, failure_type) VALUES(%s, %s, %s, %s);"""
    values = (data.get('unit_number'), data.get('time_in_cycles'), data.get('phase'), data.get('failure_type'))
    execute_query(sql, values)
